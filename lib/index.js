...
exports.register = (server, options, next) => {
  ...
  server.ext('onPreStart', (extServer, extNext) => {
    return options.redisClient.defineCommand('rateLimit', {
      numberOfKeys: 1,
      lua: SCRIPT
    })
    .then(() => {
  ...
  server.ext('onPostAuth', (request, reply) => {
    ...
    return options.redisClient.rateLimit(key, rate.window)
    .then((cache) => {
      const remaining = rate.limit - cache[0];

      rate.remaining = Math.max(remaining, 0);
      rate.reset = Math.floor(new Date().getTime() / 1000) + cache[1];
      ...
      if (remaining < 0) {
        return reply(options.overLimitError(rate, request));
      }
